...
test.describe('Mobile Responsive Layout', () => {
  ...
  test('map takes full screen on mobile with header offset', async ({ page }) => {
  ...
  test('opens bottom sheet when menu button is clicked', async ({ page }) => {
    ...
    await expect(page.locator('text=Tap a charger to see nearby spots')).toBeVisible()
  })
  ...
  test('place type filters are horizontally scrollable on mobile', async ({ page }) => {
  ...
  test('touch gestures work on bottom sheet', async ({ page }) => {
    ...
    const dragHandle = page.locator('div[class*="h-1 w-12 rounded-full bg-gray-300"]')
...
test.describe('Tablet Responsive Layout', () => {
  ...
  test('displays desktop sidebar on tablet', async ({ page }) => {
    ...
    const mobileHeader = page.locator('div[class*="md:hidden"]').filter({ hasText: 'Tesla SC Finder' })
