...
export const MobileBottomSheet: React.FC<MobileBottomSheetProps> = ({
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 md:hidden">
      ...
      {/* Bottom Sheet */}
      <div 
        className={`relative bg-white rounded-t-2xl shadow-2xl transition-all duration-300 ${
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h3 className="font-medium text-base">{charger.name}</h3>
                  <p className="text-sm text-gray-600 mt-1">{charger.city}</p>
                  ...
                  <div className="mt-2 flex items-center gap-3 text-sm">
                    ...
                    {charger.distance !== null && charger.distance !== undefined && (
                ...
                <div className="ml-3">
                  <span className={`inline-block h-2 w-2 rounded-full ${
                    charger.status === 'active' ? 'bg-green-500' : 'bg-gray-400'
                  }`} />
                </div>
              </div>
            </div>
